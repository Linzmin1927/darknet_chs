...
void forward_network(network *netp)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        // 获取当前层
        layer l = net.layers[i];
        // 如果当前层的l.delta已经动态分配了内存，则调用fill_cpu()函数，
        // 将其所有元素的值初始化为0
        if(l.delta){
        ...
        // 完成某一层的推理时，置网络的输入为当前层的输出（这将成为下一层网络的输入），
		// 要注意的是，此处是直接更改指针变量net.input本身的值，也就是此处是通过改变指
		// 针net.input所指的地址来改变其中所存内容的值，并不是直接改变其所指的内容而指
		// 针所指的地址没变，所以在退出forward_network()函数后，其对net.input的改变都将
		// 失效，net.input将回到进入forward_network()之前时的值。
        net.input = l.output;
        if(l.truth) {
            net.truth = l.output;
        }
...
void backward_network(network *netp)
{
    ...
    for(i = net.n-1; i >= 0; --i){
		...
		// 就是直接与输入层相连的第一层隐含层（注意不是输入层，我理解的输入层
		// 就是指输入的图像数据，严格来说，输入层不算一层网络，因为输入层没有
		// 训练参数，也没有激活函数），这个时候，不需要else中的赋值，1）对于
		// 第1层来说，其前面已经没有网络层了（输入层不算），因此没有必要再计算
		// 前一层的参数，故没有必要在获取上一层；2）第一层的输入就是图像输入，
		// 也即整个net最原始的输入，在开始进行反向传播之前，已经用orig变量保存了
        // 最为原始的net，所以net.input就是第一层的输入，不需要通过net.input=prev.output
		// 获取上一层的输出作为当前层的输入；3）同1），第一层之前已经没有层了，
        // 也就不需要计算上一层的delta，即不需要再将net.delta链接到prev.delta，
		// 此时进入到l.backward()中后，net.delta就是NULL（可以参看network.h中关
		// 于delta的注释），也就不会再计算上一层的敏感度了（比如卷积神经网络中
		// 的backward_convolutional_layer()函数）
        if(i == 0){
            net = orig;
        }else{
            // 获取上一层
            layer prev = net.layers[i-1];
            // 上一层的输出作为当前层的输入（下面l.backward()会用到，具体是在计
			// 算当前层权重更新值时要用到）
        ...
        // 置网络当前活跃层为当前层，即第i层
        net.index = i;
        // 反向计算第i层的敏感度图、权重及偏置更新值，并更新权重、偏置
		//（同时会计算上一层的敏感度图，存储在net.delta中，但是还差一个
		// 环节：乘上上一层输出对加权输入的导数，也即上一层激活函数对加
		// 权输入的导数）
        l.backward(l, net);
    }
}
...
{
    // 事实上对于图像检测而言，d.X.rows/net.batch=net.subdivision，
    ...
    for(i = 0; i < n; ++i){
        ...
        // 一次反向过程：计算敏感度、权重更新值、偏置更新值；适时更新过程：更新权重与偏置        
        float err = train_network_datum(net);
        sum += err;
    }
    return (float)sum/(n*batch);
